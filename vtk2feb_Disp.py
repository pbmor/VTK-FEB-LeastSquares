...
def VTK2Feb_Func(DataDir,Fname,C,nF,STJ_WallMotion,VAJ_WallMotion,STJ_Id,VAJ_Id,Circ,Long,Norm,PressureChoice,ProfileChoice,ModelChoice,FibChoice,CF,SetP):
    '''
    ...
    if ProfileChoice == 'Triangle':
        PressurePar =  C[nC:nC+1]
        nC += 1
    if ProfileChoice == 'Step':
        PressurePar =  C[nC:nC+2]
        nC += 2
    if ProfileChoice == 'SmoothStep':
        PressurePar =  C[nC:nC+4]
        nC += 4
    ...
    if ModelChoice == 'MR':
        ...
        for i in range(num_cells):
            ...
            f.write('\t\t</material>\n')
    elif ModelChoice == 'HGO' or ModelChoice == 'SetHGO':
        for i in range(num_cells):

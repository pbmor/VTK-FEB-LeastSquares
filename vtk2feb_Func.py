...
def VTK2Feb_Func(DataDir,Fname,Nf,FT,STJ_WallMotion,VAJ_WallMotion,STJ_Id,VAJ_Id):
    ...
    PressureInterpStan = 10*np.subtract(PressureInterp,np.min(PressureInterp))/np.max(np.subtract(PressureInterp,np.min(PressureInterp)))
    ...
    f.write('\t\t</ElementData>\n')
    # for i in range(Nf):
    #     f.write('\t\t<mesh_data param="fem.bc_prescribed['+str(i)+']">\n')
    #     k=0
    #     for j in STJ_Id[i]:
    #         f.write('\t\t\t<node lid="'+str(j)+'">'+str(STJ_WallMotion[i,k,0])+','+str(STJ_WallMotion[i,k,1])+','+str(STJ_WallMotion[i,k,2])+'</node>\n')
    #         k+=1
    #     k=0
    #     for j in VAJ_Id[i]:
    #         f.write('\t\t\t<node lid="'+str(j)+'">'+str(VAJ_WallMotion[i,k,0])+','+str(VAJ_WallMotion[i,k,1])+','+str(VAJ_WallMotion[i,k,2])+'</node>\n')
    #         k+=1
    #     f.write('\t\t</mesh_data>\n')
    f.write('\t</MeshData>\n')
